...
-----------------------------------------------------------------------------
-- |
-- Module      :  Geometry.CubicSpline.Boehm
-- Copyright   :  (c) 2015-2017 diagrams team (see LICENSE)
...
bsplineToBeziers controls = beziers
  where
    n                            = length controls
    numKnots                     = n + 2
    knots                        = take numKnots $ iterate (+1/(fromIntegral numKnots - 1)) 0
...
--   points.
bspline :: (InSpace v n t, FromTrail t, OrderedField n) => BSpline v n -> t
