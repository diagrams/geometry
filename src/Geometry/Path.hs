...
pathTrails = F.toList . op Path

instance (Show1 v, Show n, Unboxable v n) => Show (Path v n) where
...
instance FromTrail (Path v n) where
  fromTrail = Path . Seq.singleton
  {-# INLINE fromTrail #-}

instance Unboxable v n => Transformable (Path v n) where
  transform = over _Wrapped . fmap . transform

instance Unboxable v n => Enveloped (Path v n) where
  getEnvelope = F.foldMap getEnvelope . op Path

instance Unboxable v n => Juxtaposable (Path v n) where
...
--
--   Note that this class is very different from 'FromTrail'. 'FromTrail' is
...
-- $construct
-- Since paths are 'FromTrail', any function producing a 'FromTrail'
...
--   'explodeTrail').
-- explodePath :: (V t ~ v, N t ~ n, FromTrail t) => Path v n -> [[t]]
