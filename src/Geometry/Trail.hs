...
module Geometry.Trail
  ...
  , SegTree(..), trailMeasure, numSegs, smOffset
...
newtype ArcLength n
  = ArcLength (Sum (I.Interval n), n -> Sum (I.Interval n))

instance Wrapped (ArcLength n) where
  type Unwrapped (ArcLength n) = (Sum (I.Interval n), n -> Sum (I.Interval n))
...
instance (Metric v, OrderedField n)
    => FT.Measured (SegMeasure v n) (SegMeasure v n) where
  measure = id

instance (OrderedField n, Metric v)
    => FT.Measured (SegMeasure v n) (Segment Closed v n) where
  measure s = (SegCount . Sum) 1
            ...
            *: OffsetEnvelope (TotalOffset . view offset $ s)
                              (getEnvelope s)
...
newtype SegTree v n = SegTree (FingerTree (SegMeasure v n) (Segment Closed v n))
  deriving (Eq, Show, Monoid, FT.Measured (SegMeasure v n))

instance (Metric v, HasLinearMap v, OrderedField n) => Transformable (SegTree v n) where
  transform t (SegTree s) = SegTree $ FT.fmap' (transform t) s
...
instance (Metric v, OrderedField n, Real n)
  ...
  atParam t p = smOffset . fst $ splitAtParam t p
...
smOffset :: ( OrderedField n, Metric v,
            FT.Measured (SegMeasure v n) t
          )
       => t -> v n
smOffset = trailMeasure zero (op TotalOffset . view oeOffset)
...
instance (Show1 v, Show n) => Show (Trail' l v n) where
  showsPrec d (Line (SegTree ft)) = showParen (d > 10) $
    showString "lineFromSegments " . liftShowList showsPrec showList (F.toList ft)

  showsPrec d (Loop (SegTree ft) o) = showParen (d > 10) $
    showString "loopFromSegments " . liftShowList showsPrec showList (F.toList ft) .
...
instance (Metric v, OrderedField n) => Parametric (GetSegment (Trail' Line v n)) where
  atParam (GetSegment (Line (SegTree ft))) p
    ...
    | p >= 1 = case FT.viewr ft of
        ...
        ft' FT.:> seg -> GetSegmentCodomain $ Just (smOffset ft', seg, reparam (n-1))

    | otherwise
    = let (before, after) = FT.split ((p*n <) . numSegs) ft
      in  case FT.viewl after of
            EmptyL   -> GetSegmentCodomain Nothing
            seg FT.:< _ -> GetSegmentCodomain $ Just (smOffset before, seg, reparam (numSegs before))
    where
      n = numSegs ft
    ...
    = case FT.viewr ft of
        ...
        ft' FT.:> seg ->
          ...
          in  GetSegmentCodomain $
                Just (smOffset ft', seg, iso (subtract (n-1) . (*n))
                                         ((/n) . (+ (n-1)))
                     )
...
instance (Show1 v, Show n) => Show (Trail v n) where
  showsPrec d (Trail t) = showParen (d > 10) $
    showString "Trail " . showsPrec 11 t
...
lineOffsets = map (view offset) . lineSegments
...
segmentPoints p = scanl (.+^) p . map (view offset)
...
fixTrail t = map (review fixed) (trailLocSegments t)
...
reverseLine :: (Metric v, OrderedField n)
            => Trail' Line v n -> Trail' Line v n
reverseLine = onLineSegments (reverse . map reversing)
