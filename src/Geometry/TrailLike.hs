...
module Geometry.TrailLike
  ...
  , fromSegments, fromLocSegments
  , fromOffsets, fromLocOffsets
  , fromVertices
  , (~~), explodeTrail
...
class (Metric (V t), OrderedField (N t)) => TrailLike t where
  ...
  -- | Convert a located trail to a 'TrailLike'.
  trailLike
    :: Located (Trail (V t) (N t))
    -- ^ The concretely located trail. Note that some trail-like things
    --   (e.g. 'Trail's) may ignore the location.
    -> t
...
--   >   # centerXY # pad 1.1
(~~) :: (InSpace v n t, TrailLike t) => Point v n -> Point v n -> t
...
explodeTrail :: (InSpace v n t, TrailLike t) => Located (Trail v n) -> [t]
