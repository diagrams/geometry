...
data PolyType n
  ...
  -- ^ A \"polar\" polygon.
  --
  --   * The first argument is a list of /central/ /angles/ from each
  --     vertex to the next.
  --
  --   * The second argument is a list of /radii/ from the origin to
  --     each successive vertex.
  --
  --   To construct an /n/-gon, use a list of /n-1/ angles and /n/
  --   radii. Extra angles or radii are ignored.
  --
  --   Cyclic polygons (with all vertices lying on a circle) can be
  --   constructed using a second argument of @(repeat r)@.

  | PolySides [Angle n] [n]
  -- ^ A polygon determined by the distance between successive vertices
  --   and the external angles formed by each three successive vertices.
  --   In other words, a polygon specified by \"turtle graphics\": go
  --   straight ahead x1 units; turn by external angle a1; go straight
  --   ahead x2 units; turn by external angle a2; etc. The polygon will
  --   be centered at the /centroid/ of its vertices.
  --
  --   * The first argument is a list of /vertex/ /angles/, giving the
  --     external angle at each vertex from the previous vertex to the
  --     next.  The first angle in the list is the external angle at the
  --     /second/ vertex; the first edge always starts out heading in
  --     the positive y direction from the first vertex.
  --
  --   * The second argument is a list of distances between successive
  --     vertices.
  --
  --   To construct an /n/-gon, use a list of /n-2/ angles and /n-1/
  --   edge lengths.  Extra angles or lengths are ignored.

  | PolyRegular Int n
  -- ^ A regular polygon with the given number of sides (first argument)
  --   and the given radius (second argument).

-- | Determine how a polygon should be oriented.
data PolyOrientation n
  = NoOrient        -- ^ No special orientation; the first vertex will
                    --   be at (1,0).  This is the default.
  | OrientH         -- ^ Orient /horizontally/, so the bottommost edge
                    --   is parallel to the x-axis.
  | OrientV         -- ^ Orient /vertically/, so the leftmost edge is
                    --   parallel to the y-axis.
  | OrientTo (V2 n) -- ^ Orient so some edge is /facing/ /in/ /the/
                    --   /direction/ /of/, that is, perpendicular to,
                    --   the given vector.
...
polyTrail :: (Unbox n, OrderedField n) => PolygonOpts n -> Located (Trail V2 n)
polyTrail po = transform ori tr where
...
polygon :: (InSpace V2 n t, Unbox n, OrderedField n, FromTrail t) => PolygonOpts n -> t
polygon = fromTrail . polyTrail

-- | Generate the located trail of a polygon specified by polar data
--   (central angles and radii). See 'PolyPolar'.
polyPolarTrail :: (Unbox n, OrderedField n) =>  [Angle n] -> [n] -> Located (Trail V2 n)
...
polySidesTrail :: (Unbox n, OrderedField n) =>  [Angle n] -> [n] -> Located (Trail V2 n)
...
polyRegularTrail :: (Unbox n, OrderedField n) => Int -> n -> Located (Trail V2 n)
...
--   perpendicular to @v@.
orient :: (Unbox n, OrderedField n) => V2 n -> Located (Trail V2 n) -> Transformation V2 n
orient v = orientPoints v . fromTrail -- trailVertices
...
orientPoints v xs  = rotation a
  where
    (n1,x,n2) = maximumBy (comparing (distAlong v . view _2))
    ...
    angleFromNormal o
      ...
      where
        ...
        -- phi :: Angle n
        phi
          | theta <= pi/2 = pi/2 - theta @@ rad
          | otherwise     = theta - pi/2 @@ rad
...
--   let go 0 gs = splitParts gs
--       go i gs = do
--         rs <- markRho i gs
--         go (i - 1) rs

--       markRho i rs = do
--         isMarked <- M.read marks i
--         if isMarked
--           then pure rs
--           else do M.write marks i True
--                   markRho (f_n i) rs
...
data StarOpts
  ...
  --   function that maps each vertex index to the index of the vertex
  --   that should come next.  Indexing of vertices begins at 0.

  | StarSkip Int
  -- ^ Specify a star polygon by a \"skip\".  A skip of 1 indicates a
  --   normal polygon, where edges go between successive vertices.  A
  --   skip of 2 means that edges will connect every second vertex,
  --   skipping one in between.  Generally, a skip of /n/ means that
  --   edges will connect every /n/th vertex.

-- | Create a generalized /star/ /polygon/.  The 'StarOpts' are used to
--   determine in which order the given vertices should be connected.
--   The intention is that the second argument of type @[Point v]@ could
--   be generated by a call to 'polygon', 'regPoly', or the like, since
--   a list of vertices is 'FromTrail'.  But of course the list can be
--   generated any way you like.  A @'Path' 'v'@ is returned (instead of
--   any 'FromTrail') because the resulting path may have more than one
--   component, for example if the vertices are to be connected in
--   several disjoint cycles.
star :: (Unbox n, OrderedField n) => StarOpts -> [Point V2 n] -> Path V2 n
